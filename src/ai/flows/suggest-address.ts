...
const SuggestAddressInputSchema = z.object({
  query: z.string().describe('A partial address string.'),
});
export type SuggestAddressInput = z.infer<typeof SuggestAddressInputSchema>;

const SuggestAddressOutputSchema = z.object({
  suggestions: z.array(z.string()).describe('A list of suggested complete addresses.'),
});
...
const prompt = ai.definePrompt({
  ...
  prompt: `You are an address autocompletion service. Given a partial address query, provide a list of up to 5 likely complete address suggestions. Only return real addresses.
